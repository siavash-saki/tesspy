...
def simple_gdf():
    ...
    return gpd.GeoDataFrame({"tile_id": ["tile0"], "geometry": [poly]}, crs="EPSG:4326")
...
def test_invalid_poi_categories_type_raises(area_gdf, simple_gdf):
    ...
    with pytest.raises(ValueError, match="poi_categories must be a string or list"):
        count_poi_per_tile(city=area_gdf, gdf=simple_gdf, poi_categories=42)
...
def test_string_poi_category_accepted(area_gdf, simple_gdf, monkeypatch):
    ...
    monkeypatch.setattr("tesspy.data._geo.POIdata.get_poi_data", fake_get_poi_data)
    ...
    with pytest.raises(ValueError, match="stubbed"):
        count_poi_per_tile(city=area_gdf, gdf=simple_gdf, poi_categories="amenity")

...
The package is currently maintained by [@siavash-saki](http://github.com/siavash-saki) and [@JoHamann](http://github.com/JoHamann).
...
```python
ffm_hexagons = ffm.hexagons(resolution=7)
```
...
```python
ffm_cb = ffm.city_blocks(poi_categories=['amenity'])
```
